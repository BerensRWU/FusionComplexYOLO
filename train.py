...
def main_worker(gpu_idx, configs):
    ...
    if configs.disturb_types_training_radar[0] == "None" and configs.disturb_types_training_lidar[0] == "None":
        dist_str = 'trained_disturbances_none/'
    elif configs.disturb_types_training_radar[0] != "None" and configs.disturb_types_training_lidar[0] == "None":
        dist_str = f'trained_disturbances_radar/prob_{configs.dist_prop_radar}/level_{str(configs.disturb_levels_training_radar[0]).replace(" ", "_")}/{configs.disturb_types_training_radar[0].replace(" ", "_")}/'
    elif configs.disturb_types_training_radar[0] == "None" and configs.disturb_types_training_lidar[0] != "None":
        dist_str = f'trained_disturbances_lidar/prob_{configs.dist_prop_lidar}/level_{str(configs.disturb_levels_training_lidar[0]).replace(" ", "_")}/{configs.disturb_types_training_lidar[0].replace(" ", "_")}/'
    else:
        raise NotImplementedError 

    configs.saved_fn = configs.saved_fn.split("/")[2].split("_")[0]
    configs.checkpoints_dir = "./checkpoints/"
    configs.saved_fn = f'{configs.saved_fn}_{configs.dataset}_{sensor}/{dist_str}/'
    print(configs.checkpoints_dir, configs.saved_fn)

    os.makedirs(f"{configs.checkpoints_dir}/{configs.saved_fn}", exist_ok = True)
