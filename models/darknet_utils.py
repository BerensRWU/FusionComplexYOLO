...
def print_cfg_latex(blocks):
    print('\\begin{table}[h]\n\
    \\centering \n\
    \\caption{YOLO Netzwerk Backbone} \n\
    \\label{tab:yolo_backbone} \n\
    \\resizebox{\textwidth}{!}{% \n\
    ...
    for block in blocks:
        ...
        if block['type'] == 'net':
            ...
            width = (prev_width + 2 * pad - kernel_size) // stride + 1
            height = (prev_height + 2 * pad - kernel_size) // stride + 1
            print(f'{ind:5d} & Convolutional Layer & {filters:4d} & {kernel_size:d} x {kernel_size:d} / {stride:d} &  {prev_width:3d} x {prev_height:3d} x {prev_filters:4d} & {width:3d} x {height:3d} x {filters:4d}\\\\') 
            prev_width = width
            prev_height = height
            prev_filters = filters
            out_widths.append(prev_width)
            ...
            width = prev_width // stride
            height = prev_height // stride
            print(f'{ind:5d} & Max Pooling Layer   &      & {pool_size:d} x {pool_size:d} / {stride:d} &  {prev_width:3d} x {prev_height:3d} x {prev_filters:4d} & {width:3d} x {height:3d} x {filters:4d}\\\\')
            prev_width = width
            prev_height = height
            prev_filters = filters
            out_widths.append(prev_width)
            ...
            width = prev_width * stride
            height = prev_height * stride
            print(f'{ind:5d} & Upsample & & * {stride:d} &  {prev_width:3d} x {prev_height:3d} x {prev_filters:4d}   ->   {width:3d} x {height:3d} x {filters:4d}')
            prev_width = width
            prev_height = height
            prev_filters = filters
            out_widths.append(prev_width)
            ...
            layers = [int(i) if int(i) > 0 else int(i) + ind for i in layers]
            if len(layers) == 1:
                print(f'{ind:5d} & Route Layer & {layers[0]:d} & & &\\\\')
                prev_width = out_widths[layers[0]]
                prev_height = out_heights[layers[0]]
                prev_filters = out_filters[layers[0]]
            elif len(layers) == 2:
                print(f'{ind:5d} & Route Layer & {layers[0]:d} & {layers[1]:d}&&\\\\')
                prev_width = out_widths[layers[0]]
                prev_height = out_heights[layers[0]]
                assert (prev_width == out_widths[layers[1]])
                assert (prev_height == out_heights[layers[1]])
                ...
                print(f'{ind:5d} & Route Layer & {layers[0]:d} & {layers[1]:d} & {layers[2]:d} & {layers[3]:d}\\\\')
                prev_width = out_widths[layers[0]]
                prev_height = out_heights[layers[0]]
                assert (prev_width == out_widths[layers[1]] == out_widths[layers[2]] == out_widths[layers[3]])
                assert (prev_height == out_heights[layers[1]] == out_heights[layers[2]] == out_heights[layers[3]])
            ...
            print(f'{ind:5d} & Detection Complex-YOLO Header & & & &')
            out_widths.append(prev_width)
            out_heights.append(prev_height)
            out_filters.append(prev_filters)
        elif block['type'] == 'shortcut':
            from_id = int(block['from'])
            from_id = from_id if from_id > 0 else from_id + ind
            print(f'{ind:5d} & Skip Connection {from_id:d}& & & &')
            prev_width = out_widths[from_id]
            prev_height = out_heights[from_id]
            prev_filters = out_filters[from_id]
            out_widths.append(prev_width)
...
if __name__ == '__main__':
    ...
    blocks = parse_cfg(configs.cfgfile)
    if len(sys.argv) == 2:
