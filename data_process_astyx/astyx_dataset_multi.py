    ...
    def load_img_with_targets(self, index):
        ...
        if self.low_fusion:
            ...
            if self.disturb_lidar_data:
                if self.mode == "train":
                    if np.random.uniform(low=0.0, high=1.0) < dist_prop_lidar:
                        lidarData = self.disturb_pc(pcData=lidarData, types=np.random.choice(["random_loss","random_shift","random_noise","blobs","intensity","snow","fog"],size = 1),
                                                    levels=[float(np.random.choice([0.25,0.5,0.75,1.0],size = 1))], sample_id=sample_id, calib=calib, radar = False)
            ...
            if self.disturb_radar_data:
                if self.mode == "train":
                    if np.random.uniform(low=0.0, high=1.0) < dist_prop_radar:
                        radarData = self.disturb_pc(pcData=radarData, types=np.random.choice(["random_loss","random_shift","random_noise","blobs","intensity"],size = 1),
                                                    levels=[float(np.random.choice([0.25,0.5,0.75,1.0],size = 1))], sample_id=sample_id, calib=calib, radar = True)
                else:
            ...
            if self.disturb_radar_data:
                if self.mode == "train":
                    if np.random.uniform(low=0.0, high=1.0) < dist_prop_radar:
                        pcData = self.disturb_pc(pcData=pcData, types=np.random.choice(["random_loss","random_shift","random_noise","blobs","intensity"],size = 1),
                                                    levels=[float(np.random.choice([0.25,0.5,0.75,1.0],size = 1))], sample_id=sample_id, calib=calib, radar = True)
                else:
                        pcData = self.disturb_pc(pcData=pcData, types=self.dist_types_radar, levels=self.dist_levels_radar, sample_id=sample_id, calib=calib, radar = True)
            ...
            if self.disturb_lidar_data:
                if self.mode == "train":
                    if np.random.uniform(low=0.0, high=1.0) < dist_prop_lidar:
                        pcData = self.disturb_pc(pcData=pcData, types=np.random.choice(["random_loss","random_shift","random_noise","blobs","intensity","snow","fog"],size = 1),
                                                    levels=[float(np.random.choice([0.25,0.5,0.75,1.0],size = 1))], sample_id=sample_id, calib=calib, radar = False)
